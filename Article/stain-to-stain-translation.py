...
def summarize_performance(epoch, batch, g_model, n_samples=3):
    ...
    path_model = 'C:/Users/mirim/PycharmProjects/STST_replication/models/'
    ...
    for i in range(n_epochs):
        ...
        for batch_i, (X_realB, X_realA) in enumerate(dl.load_batch_train(n_batch)):
            ...
            print('>step: %d >epoch %d-%d >batch %d-%d, D_loss_real[%.3f]  D_loss_fake[%.3f]  G_loss[%.3f]  time: %s'
                  % ((batch_i + 1 + (i * bat_per_epo)), i + 1, n_epochs, batch_i + 1, bat_per_epo, d_loss1, d_loss2,
                     g_loss, elapsed_time))
            ...
            #%%%%%%%%%%%%%%%%%%%%%%%%% adding following block of code in order to plot loss values to use in "plot_disc_gen_loss" %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            #%%%%%%%%%%%%%%%%%%%%%%%% haven't tried running yet- not sure if works %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            # Save the loss values to NumPy files
            np.save('/STST_replication/models_loss/Disc_loss_real.npy', Disc_loss_real)
            np.save('/STST_replication/models_loss/Disc_loss_fake.npy', Disc_loss_fake)
            np.save('/STST_replication/models_loss/Gen_loss.npy', Gen_loss)
