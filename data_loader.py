...
def load_batch_test(batch_size=1, is_testing=False):
    ...
    path = glob('C:/Users/mirim/PycharmProjects/STST_replication/%s/%s/*' % (dataset_name, data_type))
...
def load_batch_train(batch_size=1, is_testing=False):
    ...
    for i in range(n_batches):
        ...
        for img in batch:
            ...
            #img_A = scipy.misc.imresize(img_A, img_res)
            #img_B = scipy.misc.imresize(img_B, img_res)
            img_A = resize(img_A, img_res, mode='reflect', anti_aliasing=True)
            img_B = resize(img_B, img_res, mode='reflect', anti_aliasing=True)
            if not is_testing and np.random.random() > 0.5:
                img_A = np.fliplr(img_A)
                img_B = np.fliplr(img_B)
            imgs_A.append(img_A)
